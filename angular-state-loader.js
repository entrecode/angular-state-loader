...
angular.module('ec.stateloader', []).directive('stateLoader', function($rootScope, $timeout) {
  ...
  return {
    scope:      {
      fromState: '@', //restrict showing loader only when loading from this state name
      toState:   '@', //restrict showing loader only when loading this state name
    ...
    compile:    function(elem, attrs, transcludeFn) {
      ...
      return {
        pre:  function preLink(scope) {
          scope.shouldFire = function(fromState, toState) {
            return (!scope.fromState || fromState.name === scope.fromState) &&
              (!scope.toState || toState.name === scope.toState) && scope.forceShow === undefined;
          };
        ...
        post: function postLink(scope, element) {
          scope.delay = typeof scope.delay === 'number' ? scope.delay : 100;
    ...
    template:   '<div class="angular-state-loader"><span ng-hide="transcluding">' +
                '<i class="material-icons rotate-right">autorenew</i>' + '</span><ng-transclude></ng-transclude></div>'
  };
});